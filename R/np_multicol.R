...
#' 
#' rmse(model = mt_m, main_effect = "wt", covariates = mt_vars)
...
np_multicol <- function(model, main_effect, covariates){
  ...
  z <- map_df(.x = covariates, .f = model_reduction_function, 
              model = model, main_effect = main_effect)
  return(z)
}

model_reduction_function <- function(model, main_effect, single_covariate){
  beta_ful <- coef(model)[main_effect]
  se_ful <- coef(summary(model))[, "Std. Error"][main_effect]
  m2 <- update(model, glue::glue(". ~ . -{single_covariate}"))
  beta_red <- coef(m2)[main_effect]
  se_red <- coef(summary(m2))[, "Std. Error"][main_effect]
